...
SCENARIO("random sample should produce correct delay", "[delay_estimation]") {
  GIVEN("there is a random sample") {
    ...
    std::vector<std::vector<std::vector<float>>> render(
        ...
        std::vector<std::vector<float>>(
            kNumRenderChannels, std::vector<float>(webrtc::kBlockSize)));
    // Capture buffer
    ...
    for (auto down_sampling_factor : kDownSamplingFactors) {
      ...
      WHEN("the down sampling factor is " +
           std::to_string(down_sampling_factor)) {
        for (auto delay_samples : kDelaySamples) {
          WHEN("the delay sample count is " + std::to_string(delay_samples)) {
            ...
            for (size_t i = 0; i < 500 + delay_samples / webrtc::kBlockSize;
                 i++) {
            ...
            THEN("there is an accurate estimation of delay") {
